...
//        return nodeRepository.findAll().stream().map(this::fillByGrpcDomainQuery).collect(Collectors.toList());
     ...
     */
    private  String getNetAddress() {
        // 从环境变量获取KUSCIA_PROTOCOL, NET_ADDRESS, KUSCIA_GATEWAY_PORT的值
        String protocol = System.getenv("KUSCIA_PROTOCOL");
        String ip = System.getenv("NET_ADDRESS");
        String port = System.getenv("KUSCIA_GATEWAY_PORT");
        // 根据KUSCIA_PROTOCOL的值设置type参数
        String protocolType;
        if ("notls".equalsIgnoreCase(protocol)) {
            protocolType = "http://"; // 如果协议是notls，使用http
        } else if ("tls".equalsIgnoreCase(protocol) || "mtls".equalsIgnoreCase(protocol)) {
            protocolType = "https://"; // 如果协议是tls或mtls，使用https
        } else {
            protocolType = "http://"; // 默认情况或其他值，使用http
        }
        String netAddress = protocolType + ip + ":" + port;
        return netAddress;
    }
    ...
    private void initPrivate(String nodeId, String net_address, String certText) {
        ...
        node_value.put("name", nodeId);
        node_value.put("dstNodeId", nodeId);
        node_value.put("dstNetAddress", net_address);
        node_value.put("certText", certText);
        node_value.put("masterNodeId", "master");
        node_info.put("value", node_value);
        data.add(node_info);
        ...
        // API endpoint URL
        String apiUrl = digitalLinkUrl + "/api/v1/messages/broadcast";
        String payload_str = JSONObject.toJSONString(payload);
        log.info("payload_str:{}", payload_str);

        String result = HttpUtil.post(apiUrl, payload_str);
        log.info("数据上链返回结果:{}", result);
    }
